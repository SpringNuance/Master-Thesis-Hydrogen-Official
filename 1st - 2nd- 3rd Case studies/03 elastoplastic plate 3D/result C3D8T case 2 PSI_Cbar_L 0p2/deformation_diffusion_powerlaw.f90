...
!====================================================================
    ...
    ! UMATHT statv
    ! "31, AR31_C_mol, AR31_C_mol   ",
    ! "32, AR32_CL_mol, AR32_CL_mol   ",
    ! "33, AR33_CT_mol, AR33_CT_mol   ",
    ! "34, AR34_CT_dis_mol, AR34_CT_dis_mol   ",
    ! "35, AR35_CT_gb_mol, AR35_CT_gb_mol   ",
    ! "36, AR36_CT_carb_mol, AR36_CT_carb_mol   ",
    ! "37, AR37_thetaL, AR37_thetaL   ",
    ! "38, AR38_thetaT_dis, AR38_thetaT_dis   ",
    ! "39, AR39_thetaT_gb, AR39_thetaT_gb   ",
    ! "40, AR40_thetaT_carb, AR40_thetaT_carb   ",
    ! "41, AR41_rho_d, AR41_rho_d   ",
    ! "42, AR42_theta_coverage, AR42_theta_coverage   ",
    ! "43, AR43_k_HEHE, AR43_k_HEHE   ",
...
module common_block
    ...
    integer, parameter :: total_elems = 7588 ! Storing the actual number of elements
    integer, parameter :: total_nodes = 15574 ! Storing the actual number of nodes
    ...
    integer, parameter :: rho_d_idx = 31
    integer, parameter :: PSI_CL_mol_idx = 32 
     
    integer, parameter :: CL_mol_idx = 33
    integer, parameter :: CT_dis_mol_idx = 34
    integer, parameter :: C_mol_idx = 35
    
    integer, parameter :: thetaL_idx = 36
    integer, parameter :: thetaT_dis_idx = 37
    ...
    integer, parameter :: nsvars = 37 ! Number of state variables in UMAT and UMATHT

    ! Technically, nmax_elems usually range from 4 to 20 for C3D8 mesh
    ! However we are uncertain what could be maximum number of elements that a node can have
    ! So we just set very large number of 100 to be safe
    ...
    ! This will store the hydrostatic stress gradient for UMATHT 
...
subroutine UMAT(stress,statev,ddsdde,sse,spd,scd,rpl,ddsddt, &
    ...
    real(kind=dp) :: E, nu, lambda, mu, eqplas, deqplas, syield, syiel0, sig_vonMises, sig_H, rhs, E_tangent, sig_flow
    real(kind=dp) :: effective_mu, effective_lambda, effective_hard, PSI_CL_mol, sig_0, n_hardening

    real(kind=dp) :: eelas(ntens), eplas(ntens), flow(ntens), hard(3), old_stress(ntens), old_eplas(ntens)
    ...
    sig_0 = props(3)       ! Initial yield strength in the absence of hydrogen (MPa)
    ...
    PSI_CL_mol_flag = props(5) ! 1 for PSI_CL_mol = 1.0, 
                               ! 2 for PSI_CL_mol = 0.2, 
    ...
    if (PSI_CL_mol_flag == 1) then
        ! First extreme case
        PSI_CL_mol = 1.0d0
    else if (PSI_CL_mol_flag == 2) then
        ! Second extreme case
        PSI_CL_mol = 0.2d0
    else if (PSI_CL_mol_flag == 3) then
        ! Third case: gradually decreasing function of Cbar_L
        ! print *, "hello"
        if (time(1) > 0) then
            PSI_CL_mol = statev(PSI_CL_mol_idx)
        else
            xi_PSI = 0.2d0
            CL_mol_max = 35.0d0
            CL_mol_min = 0.0d0
            CL_mol = 27.0d0
            PSI_CL_mol = 1.0d0 - (1.0d0 - xi_PSI) * ((CL_mol - CL_mol_min) / (CL_mol_max - CL_mol_min))
        end if
    end if


    ! From now, sig_H0 is the yield strength in the presence of hydrogen

    ! Equation 33: We assume that the current yield strength, sig_flow, 
    ! is a function of the equivalent plastic strain eqplas
    ! and Cbar_L according to the relationship
    
    sig_H0 = PSI_CL_mol * sig_0 ! Pa

    ! Initial yield strain in the absence of hydrogen
    ! eqplas_0 = sigm_H0 / E, but we expand it later instead of using it directly
    
    sig_flow = sig_H0 * (1.0d0 + E * eqplas/sig_H0) ** n_hardening 

    ! print *, sig_H0
    ! print *, sig_flow

    ! This is equivalent to 
    ! sig_flow = sig_H0 * (1 + eqplas / eqplas_0) ** n_hardening
    
    ! Determine if active yielding

    if (sig_vonMises > (1.d0 + toler) * sig_flow) then

        sig_H = (stress(1) + stress(2) + stress(3))/3.d0
        ...
        ! sig_vonMises - (3.d0 * mu * deqplas) - sig_flow = 0
        ...
        ! It is the derivative of the yield stress with respect to the plastic strain (derivative of sig_flow w.r.t eqplas)
            
        E_tangent = E * n_hardening * (1.d0 + E * eqplas/sig_H0) ** (n_hardening - 1.0d0)
        
        do k_newton = 1, newton
            rhs = sig_vonMises - (3.d0 * mu * deqplas) - sig_flow
            deqplas = deqplas + rhs / ((3.d0 * mu) + E_tangent)

            sig_flow = sig_H0 * (1.d0 + E * (eqplas + deqplas)/sig_H0) ** n_hardening
            E_tangent = E * n_hardening * (1.d0 + E * (eqplas + deqplas)/sig_H0) ** (n_hardening-1.0d0)

            if (abs(rhs) < toler * sig_H0) exit
        end do

        if (k_newton == newton) write(7,*)'WARNING: plasticity loop failed'

        ! Update stresses and strains
        stress(1:ndi) = flow(1:ndi) * sig_flow + sig_H
        ...
        stress(ndi+1:ntens) = flow(ndi+1:ntens) * sig_flow
        ...
        do i = 1, ntens
            spd = spd + (stress(i) + old_stress(i)) * (eplas(i) - old_eplas(i))/3.d0
        end do

        ! Formulate the jacobian (material tangent)   

        ! effective shear modulus
        effective_mu = mu * sig_flow / sig_vonMises 
...
subroutine UMATHT(u,dudt,dudg,flux,dfdt,dfdg, &
    ...
    PSI_CL_mol = 1.0d0 - (1.0d0 - xi_PSI) * ((CL_mol - CL_mol_min) / (CL_mol_max - CL_mol_min))
    !print *, 'PSI_CL_mol = ', PSI_CL_mol
    statev(rho_d_idx) = rho_d
    statev(PSI_CL_mol_idx) = PSI_CL_mol
    ...
    kuser_vars(noel, PSI_CL_mol_idx, npt) = PSI_CL_mol
...
!     uvar(2) = array(sig_H)
!     uvar(3) = array(sig_vonMises)
!     uvar(4) = array(triax_idx)
!     uvar(5) = array(lode_idx)
!     uvar(6) = array(C_mol_idx)
!     uvar(7) = array(CL_mol_idx)
!     uvar(8) = array(CT_mol_idx)
!     uvar(9) = array(theta_coverage_idx)
!     uvar(10) = array(k_HEDE_idx)
