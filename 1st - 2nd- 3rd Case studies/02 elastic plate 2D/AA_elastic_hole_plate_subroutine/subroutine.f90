...
subroutine calculate_grad_sigma_hydrostatic_CPE8HT(noel)
    ...
    do integration_point_id = 1, 9
        ...
        if (noel == 74) then
            print *, 'inverse_J(1,1): ', inverse_Jacobian(1,1)
            print *, 'inverse_J(1,2): ', inverse_Jacobian(1,2)
            print *, 'inverse_J(2,1): ', inverse_Jacobian(2,1)
            print *, 'inverse_J(2,2): ', inverse_Jacobian(2,2)
        end if
