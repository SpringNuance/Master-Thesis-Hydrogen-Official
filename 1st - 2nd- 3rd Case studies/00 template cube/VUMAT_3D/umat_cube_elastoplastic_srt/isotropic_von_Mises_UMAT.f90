...
subroutine UMAT(stress,statev,ddsdde,sse,spd,scd, &
    ...
    real(kind=8) :: E, nu, lambda, mu, eqplas, deqplas, syield, syiel0, sMises, shydro, rhs 
    ...
    if (sMises > (1.d0 + toler) * syiel0) then
        ...
        do k_newton = 1, newton
            rhs = sMises - (3.d0 * mu * deqplas) - syield
            deqplas = deqplas + rhs / ((3.d0 * mu) + hard(1))

            call UHARD(syield, hard, eqplas + deqplas, eqplasrt, time, dtime, temp, &
                dtemp, noel, npt, layer, kspt, kstep, kinc, cmname, nstatv, &
                statev, numfieldv, predef, dpred, nvalue, props(start_UHARD_index))
