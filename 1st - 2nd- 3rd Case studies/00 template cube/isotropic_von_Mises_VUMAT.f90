...
!
    if ( stepTime == zero ) then
        ...
        do k = 1, nblock
            ...
            ! Compute flow direction based on deviatoric stresses and von Mises stress
            ! do i = 1, ndir
            !     flow(k, i) = (stressOld(k, i) - sig_mean) / sig_vonMises
            ! end do
            
            ! ! Compute the flow direction for shear components
            ! do i = ndir + 1, ndir + nshr
            !     flow(k, i) = stressOld(k, i) / sig_vonMises
            ! end do

            ! ! Update elastic and plastic strain
            ! do i = 1, ndir
            !     eplas(k, i) = eplas(k, i) + 3.d0/2.d0 * flow(k, i) * deqplas
            !     eelas(k, i) = eelas(k, i) - 3.d0/2.d0 * flow(k, i) * deqplas
            ! end do

            ! do i = ndir + 1, ndir + nshr
            !     eplas(k, i) = eplas(k, i) + 3.d0 * flow(k, i) * deqplas
            !     eelas(k, i) = eelas(k, i) - 3.d0 * flow(k, i) * deqplas
            ! end do
            ...
            ! stateNew(k, eelas_start_idx:eelas_end_idx) = eelas(k, eelas_start_idx:eelas_end_idx)
            ! stateNew(k, eplas_start_idx:eplas_end_idx) = eplas(k, eplas_start_idx:eplas_end_idx)
...
! Write only (modifiable) variables -
    ...
    real(kind=dp) :: cond, specHeat, DL, VH, R, T, NL, CL0, mu0_L, muR_L, EB, avogadro, eqplas
    
    ! properties
    DL       =     props(1)    ! mm^2/s                   Latice Diffusivity
    VH       =     props(2)    ! mm^3/mol                 Molar Volume of Hydrogen
    R        =     props(3)    ! (Nmm) / mol·K            Gas Constant
    ...
    muR_L    =     props(8)    ! Nm/mol                   Referance Chemical Potential in Non-dimentional Modeling
    EB       =     props(9)    ! Nm/mol                   Binding Energy of Traps

    avogadro  =  6.023D+23     !                          Avogadro
    ...
    do km = 1, nblock
        ...
        stateNew(km, 4) = CL_r
        stateNew(km, 5) = KT
        stateNew(km, 6) = CL_bar
        stateNew(km, 7) = DL * xtime * CL_bar   ! Diaz2024_Eq24
        stateNew(km, 8) = xNT
        stateNew(km, 9) = xdNTdeps
        stateNew(km, 10) = Dstar
        stateNew(km, 11) = dEnerTh
        stateNew(km, 12) = dEnerThDTemp(km, 1)
        stateNew(km, 13) = xThetaT
        stateNew(km, 14) = Dstern * CL_bar * VH / muR_L
        stateNew(km, 15) = xThetaT * R * T / (CL_r * muR_L) * xdNTdeps
        stateNew(km, 16) = CL_mol
        stateNew(km, 17) = C_trap_mol
        stateNew(km, 18) = C_total
    end do
