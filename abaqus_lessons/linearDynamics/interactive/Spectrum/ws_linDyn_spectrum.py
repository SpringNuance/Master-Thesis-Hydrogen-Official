...
mdb.models.changeKey(fromName='Model-1', toName='LAYERED-BAR')
m = mdb.models['LAYERED-BAR']
...
m.FrequencyStep(name='FREQ-SIM', previous='Initial', 
    description='Frequency Extraction to 400 Hz - SIM (No Damping Projection)', 
    maxEigen=400.0, projectDamping=OFF)

# 

mdb.Model(name='SpectrumGen', modelType=STANDARD_EXPLICIT)
m = mdb.models['SpectrumGen']
...
m.materials['Material-1'].Elastic(table=((1.0, 0.3), ))
...
m.FrequencyStep(name='Step-1', previous='Initial', 
    limitSavedEigenvectorRegion=None, numEigen=2)
session.viewports['Viewport: 1'].assemblyDisplay.setValues(step='Step-1')

f1 = a.instances['Part-1-1'].faces
faces1 = f1.findAt(((3.333333, 3.333333, 20.0), ))
region = a.Set(faces=faces1, name='Face')
...
mdb.ModelFromInputFile(name='Storage-Rack', 
    inputFileName='w_storage-rack-data.inp')

m = mdb.models['Storage-Rack']
...
m.sections.changeKey(
    fromName='Section-1-TOWER_FRAME', toName='TOWER_FRAME')
m.sections.changeKey(
    fromName='Section-2-TOWER_PANEL-1', toName='TOWER_PANEL-1')
m.sections.changeKey(
    fromName='Section-3-TOWER_PANEL-2', toName='TOWER_PANEL-2')
m.sections.changeKey(
    fromName='Section-4-TOWER_PANEL-3', toName='TOWER_PANEL-3')
m.sections.changeKey(
    fromName='Section-5-DISPLAY-MASS-L1', toName='DISPLAY-MASS-L1')
m.sections.changeKey(
    fromName='Section-6-DISPLAY-MASS-L2', toName='DISPLAY-MASS-L2')
m.sections.changeKey(
    fromName='Section-7-DISPLAY-MASS-L3', toName='DISPLAY-MASS-L3')

m.parts['PART-1'].sectionAssignments[0].setValues(
    sectionName='TOWER_FRAME')

m.parts['PART-1'].sectionAssignments[1].setValues(
    sectionName='TOWER_PANEL-1')

m.parts['PART-1'].sectionAssignments[2].setValues(
    sectionName='TOWER_PANEL-2')

m.parts['PART-1'].sectionAssignments[3].setValues(
    sectionName='TOWER_PANEL-3')

m.parts['PART-1'].sectionAssignments[4].setValues(
    sectionName='DISPLAY-MASS-L1')

m.parts['PART-1'].sectionAssignments[5].setValues(
    sectionName='DISPLAY-MASS-L2')

m.parts['PART-1'].sectionAssignments[6].setValues(
    sectionName='DISPLAY-MASS-L3')

m.Material(name='DUMMY')
m.materials['DUMMY'].Density(table=((1e-20, ), ))
m.materials['DUMMY'].Elastic(table=((1.0, 0.3), ))

m.Material(name='STEEL')
m.materials['STEEL'].Density(table=((7.77e-09, ), ))
m.materials['STEEL'].Elastic(table=((200.e3, 0.3), ))

session.viewports['Viewport: 1'].setValues(displayedObject=a)

m.FrequencyStep(name='Step-1', previous='Initial', 
    description='Fixed Base Frequencies of a Storage Rack System', 
    maxEigen=40.0, projectDamping=OFF)
session.viewports['Viewport: 1'].assemblyDisplay.setValues(step='Step-1')

m.fieldOutputRequests['F-Output-1'].setValues(
    variables=('S', 'U'))

mdb.saveAs(pathName='RespSpectrum')
