...
!     store the concentration in each trap, in all traps and in traps and lattice
      id=3+2*ntens ! = 3 + 2*4 = 11
      statev(ntraps+1+id)=0 ! = 3 + 1 + 11 = 15, corresponding to C_trap, statev(15) = 0
      do k1=1,ntraps ! = 1, 2, 3
       statev(k1+id)=xNt(k1)*xK(k1)*cL/(xK(k1)*cL+xNl) ! statev(12), statev(13), statev(14) 
       statev(ntraps+1+id)=statev(ntraps+1+id)+statev(k1+id) ! statev(15) = statev(15) + statev(12 to 14)
      end do
      statev(ntraps+2+id)=cL+statev(ntraps+1+id) ! statev(16) = cL + statev(15), corresponding to C_lattice + C_trap
...
! If using this code for research or industrial purposes, please cite:
! S. del Busto, C. Beteg�n, E. Mart�nez-Pa�eda. A cohesive zone 
