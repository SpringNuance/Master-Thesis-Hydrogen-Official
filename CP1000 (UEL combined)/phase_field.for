...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    dimension rhs(mlvarx,*),amatrx(ndofel,ndofel),props(*),svars(*), &
              energy(*),coords(mcrd,nnode),u(ndofel),du(mlvarx,*),v(ndofel), &
    ...
    dimension wght(ninpt),dN(nnode,1),dNdz(ndim,nnode),stran(ntens), &
              ...
              stress(ntens),dstran(ntens),statev_local(nsvint)
...
!   enforcing Karush-Kuhn-Tucker conditions
    ...
    statev_local(2*ntens+2)=H
      
    call kstatevar(kintk,nsvint,svars,statev_local,0)
       
    amatrx(1:8,1:8)=amatrx(1:8,1:8)+ &
         dvol*(((1.d0-phin)**2+xk)*matmul(matmul(transpose(b),ddsdde),b))
        
       rhs(1:8,1)=rhs(1:8,1)- &
         dvol*(matmul(transpose(b),stress)*((1.d0-phin)**2+xk))       
           
        amatrx(9:12,9:12)=amatrx(9:12,9:12)+ &
         dvol*(matmul(transpose(dNdx),dNdx)*Gc*xlc+ &
         matmul(dN,transpose(dN))*(Gc/xlc+2.d0*H))   

        rhs(9:12,1)=rhs(9:12,1)- &
         dvol*(matmul(transpose(dNdx),matmul(dNdx,u(9:12)))*Gc*xlc+ &
         dN(:,1)*((Gc/xlc+2.d0*H)*phi-2.d0*H))
                       
! output
       user_var(jelem,1:4,kintk)=statev_local(1:ntens)*((1.d0-phin)**2+xk)
       user_var(jelem,5:9,kintk)=statev_local((ntens+1):(2*ntens+1))
...
subroutine kshapefcn(kintk,ninpt,nnode,ndim,dN,dNdz)
      ...
      parameter (gauss_coord=0.577350269d0)
      dimension dN(nnode,1),dNdz(ndim,*),coord24(2,4)
      
      data  coord24 /-1.d0, -1.d0, 1.d0, -1.d0, -1.d0, 1.d0, 1.d0, 1.d0/     
     
!     2D 4-nodes

!     determine (g,h,r)
      g=coord24(1,kintk)*gauss_coord
      h=coord24(2,kintk)*gauss_coord
