...
module common_block
    ...
    integer, parameter :: total_elems = 1499 ! Storing the actual number of elements
    integer, parameter :: total_nodes = 3196 ! Storing the actual number of nodes
    ...
    integer, parameter :: C_mol_idx = 31 
    integer, parameter :: CL_mol_idx = 32
    integer, parameter :: CT_mol_idx = 33
    integer, parameter :: CT_dis_mol_idx = 34
    integer, parameter :: CT_gb_mol_idx = 35
    integer, parameter :: CT_carb_mol_idx = 36
    integer, parameter :: thetaL_idx = 37
    integer, parameter :: thetaT_dis_idx = 38
    integer, parameter :: thetaT_gb_idx = 39
    integer, parameter :: thetaT_carb_idx = 40
    integer, parameter :: rho_d_idx = 41
    integer, parameter :: theta_coverage_idx = 42
    integer, parameter :: k_HEHE_idx = 43
    ...
    integer, parameter :: nsvars = 43 ! Number of state variables in UMAT and UMATHT
    ...
    ! This matrix keeps track of sig_H at all IP for all elements
    ...
    real(kind=dp) :: coords_all_nodes_initial(total_nodes, ndim)
...
subroutine calc_sig_H_grad_all_elems_at_inpts(noel, kinc)
    ...
    do kinpt=1, ninpt ! (8 integration points)
            ...
            do idim = 1, ndim
                do jdim = 1, ndim
                    xjac(jdim, idim) = xjac(jdim, idim) + &
                        N_grad_node_to_local_kinpt(jdim, knode) * coords_all_inpts(noel, knode, idim)
                end do  
            end do
        end do
...
subroutine UMAT(stress,statev,ddsdde,sse,spd,scd,rpl,ddsddt, &
    ...
    if (npt == 1 .and. time(1) > 0) then
        call calc_sig_H_grad_all_elems_at_inpts(noel, kinc)
    end if  
