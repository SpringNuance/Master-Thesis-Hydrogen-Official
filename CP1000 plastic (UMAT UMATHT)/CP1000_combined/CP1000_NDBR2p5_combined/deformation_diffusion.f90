...
module common_block
    ...
    integer, parameter :: total_elems = 33643 ! Storing the actual number of elements
    integer, parameter :: total_nodes = 41937 ! Storing the actual number of nodes
...
subroutine UMAT(stress,statev,ddsdde,sse,spd,scd,rpl,ddsddt, &
    ...
    real(kind=dp) :: eelas(ntens), eplas(ntens), flow(ntens)
    real(kind=dp) :: hard(3), old_stress(ntens), old_eplas(ntens)
    real(kind=dp) :: sig_principal(ndim), sig_principal_dir(ndim, ndim)
    ...
    call calc_max_principal_stress(stress, sig_principal, ntens, ndim)
    !call sprind(stress,sig_principal,sig_principal_dir,1,ndi,nshr)
    call calc_sig_Tresca(sig_principal, sig_Tresca, ndim)

    sig_P1 = sig_principal(1)
    sig_P2 = sig_principal(2)
    sig_P3 = sig_principal(3)
...
!
    ...
    uvar(4) = array(sig_P1_idx)
    uvar(5) = array(triax_idx)
    uvar(6) = array(lode_idx)
    uvar(7) = array(C_mol_idx)
    uvar(8) = array(CL_mol_idx)
    uvar(9) = array(CT_mol_idx)
